...
void test01()
{
	...
	for (int i = 0; i < 100000; i++) {
		v.push_back(i);
		if (p != &v[0]) {
			p = &v[0];		//使p总是指向新开辟内存的首位置，统计开辟空间的cu
			num++;
		}
	}

	cout << "num:" << num << endl;		//当没写v.reserve(100000); 时num:30      开辟了30次空间			写了v.reserve(100000); 后，num:1   只开辟了依次空间
}
...
int main() {
	...
	srand((unsigned int)time(NULL));	//an
