...
组件的理解：用来实现 **局部功能效果** 的 **代码和资源的集合**（html/css/js/image ...）。
...
<script type="text/babel">
        // 1.创建函数式组件 
        function Demo(){
            console.log(this); // 这个this是undefined,因为babel编译后开启了严格模式
            return <h2>我是一个函数式定义的组件，适用于【简单组件】的定义</h2>
        }
        //函数式组件还可以添加参数
        function Show(props){
        ...
        ReactDOM.render(<Show name="React" />, document.getElementById("div"));
</script>
```
...
<script type="text/babel">
        ...
        class MyComponent extends React.Component{
            render(){
                ...
                return <h1>我是一个类定义的组件，适用于【复杂组件】的定义</h1>
            }
        }
        ...
        class MyComponent2 extends React.Component{
            render(){
                return <h1>我是一个类定义的组件，适用于【复杂组件】的定义, 我是 {this.props.name}</h1>
            }
        }
        
        //2.渲染组件
        ReactDOM.render(<MyComponent />, document.getElementById("test"))
        ReactDOM.render(<MyComponent2 name="React" />, document.getElementById("test2"))
</script>
```
        ...
        class MyComponent extends React.Component{
            render(){
                ...
                let com = this.props.arr.map((item, index)=>{
                    // 嵌套 GetLi 组件
                    return <GetLi value={item} key={index} />
                });
                console.log(com);
                return <span>nums: <ul>{com}</ul> </span>
            }
        }
...
可在 [CodePen](https://codepen.io/gaearon/pen/VKQwEo?editors=1010) 上查看效果。该组件用于描述一个社交媒体网站上的评论功能，它接收 `author`（对象），`text` （字符串）以及 `date`（日期）作为 props。
...
在 [CodePen](https://codepen.io/gaearon/pen/rrJNJY?editors=1010) 上查看效果。
...
**简化**：state的赋值可以不再构造函数中进行

```js
<script type="text/babel">
        class Weather extends React.Component{
            // class 中可以直接写赋值语句，相当于实例变量（每个实例共有）
            state = {isHot:true, win:"微风"};
...
<script type="text/babel">
    ...
    // 在js中可以使用{...p}来复制一个对象，但是这个地方并不是复制对象，
    // 而是babel+react通过展开运算符，展开了一个对象，但是只能在标签中进行使用
...
React 只要保证类自身有 `propTypes` 和 `defaultProps` 属性，就能给你做限制，所以可以把这两属性使用 static 修饰，加到类里面。顺带提一下构造器中的 `props` 是否需要传递：
...
我们都说，组件 **实例** 的三大属性，有实例说明得有this（当前实例对象），所以应该只有类式组件才有这三大属性。但其实有一个特例，就是 `props`，函数式组件也可以使用。
...
在想要获取到一个DOM节点，可以直接在这个 **节点上添加ref属性**。利用该属性进行获取该节点的值。
        ...
        render(){
            return (
                <div>
                    ...
                    <br/>
                    <input ref="input2" onBlur={this.inputBlur} type="text" placeholder="失去焦点弹出弹出" />
                </div>
            )
        }
    }
...
React 也支持另一种设置 refs 的方式，称为“回调 refs”。它能助你更精细地控制何时 refs 被设置和解除。回调的这种方式会 **将该DOM作为参数传递过去**。
...
如果 `ref` 回调函数是以**内联函数（在标签中直接定义函数）的方式**定义的，在**更新（render 重新调用）过程中**它会被执行**两次**：

- 第一次传入参数 `null`，
- 第二次会传入参数 DOM 元素。

这是因为**在每次渲染时会创建一个新的函数实例**，所以 React **清空旧的 ref 并且设置新的**。通过将 ref 的回调函数定义成 class 的绑定函数的方式可以避免上述问题，但是大多数情况下它是无关紧要的。

下面把天气的demo引过来，用于重新调用render渲染，看看上面的情况。
...
<script type="text/babel">
    class Dom extends React.Component{
        ...
        //但是这个容器是独立的，所以每一个ref都需要创建这个
        MyRef = React.createRef();
        MyRef2 = React.createRef();
        
        btnOnClick = () =>{
            //创建的容器中的current属性，就是对应的DOM节点
        ...
        render(){
            return (
                <div>
                    ...
                    <button onClick = {this.btnOnClick}>点击</button>
                    <br/>
                    <input ref={this.MyRef2} onBlur={this.inputBlur} type="text" placeholder="失去焦点弹出弹出" />
                </div>
            )
        }
    }
